...
Version 2 - Fixed encoding issues
"""
...
def translate_file(input_file, output_file, file_type):
    ...
    encodings = [encoding, 'utf-8', 'latin-1', 'cp1252', 'iso-8859-1', 'utf-16', 'utf-16le']
    
    for enc in encodings:
        try:
            with open(input_file, 'r', encoding=enc, errors='ignore') as infile:
    ...
    # Now read the full file with the working encoding
    with open(input_file, 'r', encoding=enc, errors='ignore') as infile, \
    ...
    print(f"✅ Successfully translated {input_file} ({row_count} rows)")
